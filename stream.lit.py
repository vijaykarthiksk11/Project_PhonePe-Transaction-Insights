...
section = st.sidebar.selectbox(
    "Pick a category",
    ["Home", "Business Case Study", "Case Study Insights"]
)
    ...
    if sub_tab == "Transaction":
        ...
        if not df_total.empty:
            ...
            st.metric(
                ...
                f"{int(total_transactions):,}" if total_transactions is not None else "No data for insurance transactions in state and year"
            )

            st.metric(
                "Total Insurance Amount (₹)",
                f"{int(total_amount):,}" if total_amount is not None else "No data for insurance transactions in state and year"
            )
        else:
            st.metric("Total Insurance Transactions", "No data for insurance transactions in state and year")
            st.metric("Total Insurance Amount (₹)","No data for insurance transactions in state and year")
        tab1, tab2, tab3 = st.tabs(["States", "Districts", "Pincodes"])
...
# --- Case Study Insights ---
elif section == "Case Study Insights":
    ...
    if case_option == "Decoding Transaction Dynamics":
        ...
        if not df_state_yearly.empty and "TotalCount" in df_state_yearly.columns:
            ...
            st.warning("No data available for Insurance Transactions in State and Year.")
        ...
        if not df_state_yearly.empty and "TotalCount" in df_state_yearly.columns:
            ...
            st.warning("No data available for Least Penetrated States in Avg Yearly Count.")
